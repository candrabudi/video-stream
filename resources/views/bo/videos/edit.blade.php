...
@section('content')
    <div class="d-flex align-items-center justify-content-between my-4 page-header-breadcrumb flex-wrap gap-2">
        <div>
            <p class="fw-medium fs-20 mb-0">Edit Video</p>
        </div>
            ...
            <div class="card-body">
                <form id="videoForm">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="title" value="{{ $video->title }}" required>
                    </div>
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="channel_id" required>
                            ...
                            @foreach (\App\Models\Channel::orderBy('name')->get() as $channel)
                                <option value="{{ $channel->id }}"
                                    {{ $channel->id == $video->channel_id ? 'selected' : '' }}>{{ $channel->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        @if ($video->thumbnail)
                            <img src="{{ asset('storage/' . $video->thumbnail) }}" alt="thumbnail" class="mt-2"
                                width="120">
                        @endif
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Video URL</label>
                        <input type="text" class="form-control" id="video_url" value="{{ $video->video_url }}">
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Deskripsi</label>
                        <textarea class="form-control" id="description">{{ $video->description }}</textarea>
                    </div>
                    <div class="mb-3">
...
@push('scripts')
    ...
    <script>
        ...
        document.getElementById('updateBtn').addEventListener('click', function() {
            ...
            axios.post(`/videos/${id}/update`, formData, {
                    headers: {
                ...
                .catch(err => {
                    console.error(err);
                    showToast('Gagal memperbarui video', 'bg-danger');
                })
                .finally(() => {
                    this.disabled = false;
