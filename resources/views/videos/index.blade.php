            ...
            <div class="flex flex-wrap items-center justify-between gap-3">
                ...
                <div class="flex flex-wrap gap-3 items-center w-full sm:w-auto">
                    ...
                    <input type="text" id="searchInput" placeholder="Cari video..."
                        class="border rounded-lg px-3 py-2 flex-1 min-w-[250px] sm:min-w-[300px]">
        ...
        function fetchVideos(reset = false) {
            ...
            axios.get(`/videos/list?${params.toString()}`)
                ...
                <div class="p-4 flex flex-col flex-1 justify-between">
                    ...
                    <p class="text-gray-500 text-sm mb-1">Kategori: ${escapeHTML(v.category?.name ?? '-')}</p>
                    <p class="text-gray-500 text-sm mb-1">Creator: ${escapeHTML(v.creator?.full_name ?? '-')}</p>
                    ${v.video_url ? `<p class="text-blue-600 text-sm mb-1"><a href="${v.video_url}" target="_blank">Link Video</a></p>` : ''}
                ...
                .catch(err => toast('Gagal memuat video', true))
                .finally(() => loading = false);
        }

        function deleteVideo(id) {
            if (!confirm('Apakah yakin ingin menghapus video ini?')) return;
            axios.delete(`/videos/${id}/delete`)
                .then(res => {
                    toast(res.data.message || 'Video dihapus');
                    fetchVideos(true);
                })
                .catch(() => toast('Gagal menghapus video', true));
        }
